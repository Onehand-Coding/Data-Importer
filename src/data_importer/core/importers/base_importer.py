...
class ImportResult:
    ...
    def add_error(
        self, row_number: Optional[int], error: str, data_snippet: Optional[str] = None
    ):
        """Adds an error to the import results."""
        row_display = str(row_number) if row_number is not None else "Unknown"
        error_entry = {"row": row_display, "error": error, "data": data_snippet or "{}"}
        self.errors.append(error_entry)
        logger.warning(
            f"Import Error [Row: {row_display}]: {error} - Data: {data_snippet}"
        )
...
class BaseImporter(ABC):
    ...
    def _map_row(self, raw_row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for db_field, source_header in self.column_mapping.items():
            ...
            if value is not None:
                ...
                logger.debug(
                    f"Source header '{source_header}' for DB field '{db_field}' not found in raw row or value is None. Setting to None."
                )
                target_data[db_field] = None
        return target_data

    def _format_data_snippet(
    ...
    def _format_integrity_error(
        self, e: sqlite3.IntegrityError, schema_info_for_validation: Dict
    ) -> str:
        msg = str(e)
        unique_match = re.search(
            r"UNIQUE constraint failed: \w+\.(.*)", msg, re.IGNORECASE
        )
        if unique_match:
            ...
            original_column_name = next(
                (
                    sh
                    for db_f, sh in self.column_mapping.items()
                    if db_f == sanitized_db_field
                ),
                sanitized_db_field,
            )
        ...
        not_null_match = re.search(
            r"NOT NULL constraint failed: \w+\.(.*)", msg, re.IGNORECASE
        )
        if not_null_match:
    ...
    def validate_mapped_row(
        ...
        for db_field_name, value in mapped_row.items():
            original_header_name = next(
                (
                    sh
                    for db_f, sh in self.column_mapping.items()
                    if db_f == db_field_name
                ),
                db_field_name,
            )
            ...
            if db_field_name in required_fields:
                if (
                    value is None
                ):  # Checks for None, which now includes previously empty strings
                    errors.append(
                        f"Required field '{original_header_name}' is missing or empty."
                    )
                    is_valid = False
            ...
            if is_email_field:
                if (
                    ...
                    errors.append(
                        f"Invalid format for field '{original_header_name}': '{value}'."
                    )
                    is_valid = False
    ...
    def _insert_data(
        ...
        query = (
            f'INSERT INTO "{sanitized_table_name}" ({columns}) VALUES ({placeholders})'
        )
        ...
        try:
            ...
            if cursor:
                ...
                result.add_error(
                    row_number,
                    "DB insert failed (no cursor/unknown reason after execute).",
                    self._format_data_snippet(data_to_insert),
                )
                return False
        except sqlite3.IntegrityError as ie:
            logger.warning(
                f"Integrity error for table {sanitized_table_name} on row {row_number}: {ie} - Data: {data_to_insert}"
            )
            ...
            result.add_error(
                row_number, error_msg, self._format_data_snippet(data_to_insert)
            )
            return False
        except Exception as e:
            logger.exception(
                f"Unexpected database insert error for table {sanitized_table_name} on row {row_number}: {e}"
            )
            result.add_error(
                row_number,
                f"Unexpected DB Insert Error: {str(e)}",
                self._format_data_snippet(data_to_insert),
            )
            return False
    ...
    def process_import(
        ...
        try:
            for row_number, raw_row in enumerate(self.read_data(file_path), 1):
                ...
                try:
                    mapped_row = self._map_row(
                        raw_row
                    )  # This now converts empty strings to None
                    ...
                    if not mapped_row or is_empty_after_map:
                        logger.warning(
                            f"Row {row_number} resulted in empty mapped data. Skipping."
                        )
                        results.add_error(
                            row_number,
                            "Row is empty after mapping or source row was effectively empty.",
                            self._format_data_snippet(raw_row),
                        )
                        results.rows_skipped += 1
                        continue

                    # Pass schema_info_for_validation, which contains 'required', 'unique' lists
                    is_valid, validation_errors = self.validate_mapped_row(
                        mapped_row, row_number, schema_info_for_validation
                    )

                    if is_valid:
                        if not self._insert_data(
                            sanitized_table_name, mapped_row, results, row_number
                        ):
                            results.rows_skipped += 1
                    else:
                        results.add_error(
                            ...
                            self._format_data_snippet(mapped_row),
                        )
                        results.rows_skipped += 1

                except Exception as row_err:
                    logger.exception(
                        f"Unexpected error processing data for row {row_number}: {row_err}"
                    )
                    results.add_error(
                        row_number,
                        f"Unexpected row processing error: {str(row_err)}",
                        self._format_data_snippet(raw_row),
                    )
                    results.rows_skipped += 1
            ...
            if self.db_manager.connection and self.db_manager.connection.in_transaction:
                ...
                try:
                    ...
                    results.add_error(
                        None, f"Database commit error at end of import: {e}"
                    )
            ...
            logger.exception(
                f"General failure to read or process file {file_path}: {file_err}"
            )
