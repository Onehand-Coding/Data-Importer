...
from data_importer.core.importers.base_importer import BaseImporter, ImportResult
...
class CSVImporter(BaseImporter):
        ...
        try:
            ...
            if not headers:
                ...
                try:
                    ...
                    raise ValueError(
                        "CSV file appears to be empty (could not read headers)."
                    )
                except Exception as csv_err:
                    logger.error(f"CSV reader fallback failed for headers: {csv_err}")
                    raise ValueError(
                        "Could not determine headers from CSV file."
                    ) from csv_err

            if not headers:
                raise ValueError(
                    "CSV file appears to have no valid headers or is empty."
                )
    ...
    def get_preview(self, file_path: Path, num_rows: int = 5) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(
                f"Preview requested for empty file (or only headers): {file_path}"
            )
        ...
        try:
            with file_path.open("r", encoding="utf-8-sig", newline="") as csvfile:
                ...
                try:
                    ...
                    logger.info(
                        f"Detected CSV dialect: delimiter='{dialect.delimiter}', quotechar='{dialect.quotechar}'"
                    )
                except csv.Error:
                    logger.warning(
                        "Could not detect CSV dialect, falling back to standard comma delimiter."
                    )
                    csvfile.seek(0)
                ...
                if not reader.fieldnames:
                    logger.warning(
                        f"CSV file {file_path} has no field names (headers)."
                    )
                    return  # Yield nothing if no headers

                # Clean fieldnames obtained from DictReader
                cleaned_fieldnames = [
                    h.strip() for h in reader.fieldnames if h is not None
                ...
                for row_dict in reader:
                    ...
                    for k, v in row_dict.items():
                        if k is not None:  # Skip None keys
                            ...
                            logger.warning(
                                f"Row {row_count + 1}: Found unexpected None key in DictReader output."
                            )
    ...
    def import_from_file(self, file_path: Path, *args, **kwargs) -> Dict[str, Any]:
        ...
        raise NotImplementedError(
            "Direct use of import_from_file is deprecated. Use the factory/process_import."
        )
