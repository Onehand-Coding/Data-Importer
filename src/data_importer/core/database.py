...
class DatabaseManager:
    def __init__(self, db_path_str: str):
        ...
        if db_path_str == ":memory:" or db_path_str.startswith(
            "file::memory"
        ):  # More robust check
            ...
            self.db_path = Path(
                db_path_str
            )  # Keep for display/logging if needed, but don't resolve for connection
            logger.info(
                f"DatabaseManager initialized for IN-MEMORY database: {self.db_path_for_connection}"
            )
        else:
            ...
            try:
                ...
                logger.info(
                    f"DatabaseManager initialized for disk-based path: {self.db_path}"
                )
            except (
                Exception
            ) as e:  # Handle potential errors if db_path_str is not a valid path string
                logger.error(
                    f"Invalid file path string for DatabaseManager: {db_path_str} - {e}"
                )
    ...
    def connect(self) -> bool:
        try:
            ...
            self.connection = sqlite3.connect(
                self.db_path_for_connection, timeout=10, check_same_thread=False
            )
            self.connection.execute("PRAGMA foreign_keys = ON;")
            logger.info(
                f"Successfully connected to database: {self.db_path_for_connection} (SQLite v{sqlite3.sqlite_version})"
            )
            return True
        except sqlite3.Error as e:
            logger.error(
                f"Database connection error to {self.db_path_for_connection}: {e}",
                exc_info=True,
            )
            ...
            logger.error(
                f"Unexpected error connecting to {self.db_path_for_connection}: {e}",
                exc_info=True,
            )
            self.connection = None
            return False
        ...
        if self.connection:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Final commit failed before close (rolling back): {commit_err}"
                    )
                    try:
                ...
                logger.error(
                    f"Error closing database connection {self.db_path}: {e}",
                    exc_info=True,
                )
            finally:
                self.connection = None
            ...
            logger.debug(
                "Attempted to close an already closed or non-existent database connection."
            )
    ...
    def execute(
        ...
        try:
        ...
        ) as e:  # <--- MODIFIED: Catch IntegrityError specifically
            ...
            logger.warning(
                f"Integrity constraint violation: {e}. Re-raising for caller."
            )
            ...
            if self.connection:
                try:
                    if self.connection.in_transaction:
                        logger.warning(
                            "Attempting transaction rollback due to caught database error."
                        )
                        self.connection.rollback()
    ...
    def create_dynamic_table(
        self, table_name: str, schema_definition: Dict[str, str]
    ) -> bool:
        """Creates a table dynamically based on the provided schema."""
        logger.info(
            f"create_dynamic_table received call with table_name='{table_name}'"
        )
        ...
        if not sanitized_table_name:
            logger.error(
                "Table creation failed: Invalid or empty table name provided after sanitization."
            )
            return False
        if not schema_definition:
            logger.error(
                f"Table creation failed for '{sanitized_table_name}': No columns defined in schema_definition."
            )
            return False

        column_defs = []
        defined_sanitized_cols = {
            self.sanitize_name(k) for k in schema_definition.keys()
        }
        if "id" not in defined_sanitized_cols:
            column_defs.append('"id" INTEGER PRIMARY KEY AUTOINCREMENT')
            logger.info(
                f"Automatically adding 'id INTEGER PRIMARY KEY AUTOINCREMENT' to table '{sanitized_table_name}'."
            )
        ...
        for col_name, col_type in schema_definition.items():
            ...
            if not sanitized_col_name:
                logger.warning(
                    f"Skipping invalid column name '{col_name}' during table creation for '{sanitized_table_name}'."
                )
                continue
            ...
            if not validated_type:
                logger.warning(
                    f"Column type for '{sanitized_col_name}' was empty or invalid ('{col_type}'), defaulting to TEXT."
                )
                validated_type = "TEXT"
            ...
            if sanitized_col_name == "id":
                ...
                if "INTEGER PRIMARY KEY" in validated_type:
                    column_defs.append(
                        f'"{sanitized_col_name}" INTEGER PRIMARY KEY AUTOINCREMENT'
                    )  # Standardize auto-increment
                else:
        ...
        if not column_defs:
            ...
            if (
                auto_id_added
                and len(column_defs) == 1
                and column_defs[0].startswith('"id"')
            ):
                logger.error(
                    f"Table creation failed for '{sanitized_table_name}': No valid user-defined columns could be defined."
                )
                return False
            elif not column_defs:  # Should not happen if schema_definition wasn't empty
                logger.error(
                    f"Table creation failed for '{sanitized_table_name}': No columns could be defined."
                )
                return False

        sql = f'CREATE TABLE IF NOT EXISTS "{sanitized_table_name}" ({", ".join(column_defs)});'
        logger.info(
            f"Attempting to execute schema statement for table '{sanitized_table_name}':\nSQL: {sql}"
        )
        cursor = self.execute(sql, commit=True)  # Execute the CREATE TABLE command

        if cursor is not None:
            logger.info(
                f"Table '{sanitized_table_name}' created or verified successfully."
            )
            # Additionally, verify schema if table already existed? Maybe too complex for now.
            return True
        else:
            logger.error(
                f"Failed to execute schema statement for table '{sanitized_table_name}'. Check previous error log for details."
            )
            return False
