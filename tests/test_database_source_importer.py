...
from sqlalchemy.exc import SQLAlchemyError, OperationalError as SQLAlchemyOperationalError, NoSuchTableError
...
class TestDatabaseSourceImporter(unittest.TestCase):
    ...
    def test_connect_to_source_success(self):
        ...
        with patch('sqlalchemy.create_engine', return_value=real_engine) as mock_create_real, \
             patch('sqlalchemy.inspect', return_value=inspect(real_engine)) as mock_inspect_real:
    ...
    def test_get_headers_from_source_query_success(self):
        ...
        self.assertIn(query.upper(), str(executed_call_arg).upper()) # Check original query is part of it
    ...
    def test_get_preview_from_source_table(self, mock_pd_read_sql):
        ...
        called_sql_arg = mock_pd_read_sql.call_args[1]['sql'] # Get the 'sql' keyword argument
    ...
    def test_read_data_from_source_table(self):
        self.importer.source_engine = self.mock_engine
        self.mock_engine.name = "postgresql"
        mock_row1 = MagicMock(); mock_row1._asdict.return_value = {'id': 1, 'name': 'Alice'}
        mock_row2 = MagicMock(); mock_row2._asdict.return_value = {'id': 2, 'name': 'Bob'}
        mock_result_proxy = MagicMock()
    ...
    def test_map_source_row_to_target(self):
        ...
        self.assertEqual(mapped, {"target_id": 1, "target_name": "Test Name"}) # Trimmed
        ...
        self.assertEqual(mapped_empty, {"target_id": 2, "target_name": None}) # Empty becomes None
    ...
    def test_validate_target_row(self):
        ...
        self.importer.column_mapping = {"name_col": "Source Name", "email_col": "Source Email", "optional_col": "Source Optional"}
        ...
        self.assertFalse(is_valid)
        self.assertEqual(len(errors), 1)
        self.assertIn("Invalid email format for target field 'email_col' (from 'Source Email'): 'charlie.com'.", errors[0])
    ...
    def test_process_import_to_target_success_flow(
        ...
        source_id = "my_source_table_or_query"; is_query = False
        target_table = "target_sqlite_table"
        mapping_for_target = {"id_target": "src_id", "name_target": "src_name"}
        detailed_target_schema = {"id_target": {"type": "INTEGER"}, "name_target": {"type": "TEXT"}}
        schema_info_validation_for_target = {"required": [], "unique": []}

        results = self.importer.process_import_to_target(
            self.target_db_manager, source_id, is_query, target_table, mapping_for_target,
    ...
    def test_get_headers_from_source_table_not_found(self):
        self.importer.source_engine = self.mock_engine
        self.importer.source_inspector = self.mock_inspector
        self.mock_inspector.get_columns.side_effect = NoSuchTableError("Simulated table not found")
        with self.assertRaisesRegex(ValueError, "Could not get headers from source 'non_existent_table'"):
    ...
    def test_get_headers_from_source_table_inspector_returns_empty(self):
        ...
        headers = self.importer.get_headers_from_source("empty_columns_table", is_query=False)
    ...
    def test_read_data_from_source_empty_table(self):
        ...
        data_iterator = self.importer.read_data_from_source("empty_table", is_query=False, chunk_size=50)
        ...
        self.assertIsInstance(actual_arg, type(text("")), "Argument should be a SQLAlchemy TextClause")
    ...
    def test_read_data_from_source_empty_query_result(self):
        ...
        data_iterator = self.importer.read_data_from_source(query, is_query=True, chunk_size=50)
        data = list(data_iterator)
    ...
    def test_map_source_row_to_target_source_key_missing_in_row(self):
        self.importer.column_mapping = {"target_id": "src_id", "target_name": "src_name_missing"}
    ...
    def test_validate_target_row_multiple_errors(self):
        ...
        self.importer.column_mapping = {
            "email_col": "SourceEmail", "name_col": "SourceName", "age_col": "SourceAge"
        }
        schema_info_val_arg = {"required": ["name_col", "age_col"], "unique": ["email_col"]}
        invalid_row = {"email_col": "not-an-email"} # Missing name_col and age_col, invalid email
        is_valid, errors = self.importer._validate_target_row(invalid_row, 1, schema_info_val_arg)
    ...
    def test_process_import_to_target_target_db_manager_none(self):
        with self.assertRaisesRegex(ValueError, "Target DatabaseManager instance cannot be None for import process."):
    ...
    def test_process_import_to_target_no_data_from_source(self, mock_read_data):
        ...
        results = self.importer.process_import_to_target(
            self.target_db_manager, "empty_source", False, "target_sqlite_table",
            {"target_col": "src_col"}, {"target_col": {"type": "TEXT"}}, {"required": [], "unique": []}
        )
    ...
    @patch.object(DatabaseSourceImporter, '_insert_data_to_target')
    @patch.object(DatabaseSourceImporter, '_validate_target_row', return_value=(False, ["Validation error"]))
    @patch.object(DatabaseSourceImporter, '_map_source_row_to_target', return_value={"mapped_col": "mapped_value"})
    @patch.object(DatabaseSourceImporter, 'read_data_from_source')
    def test_process_import_to_target_all_rows_skipped_validation(
        ...
        results = self.importer.process_import_to_target(
            self.target_db_manager, "source_data", False, target_table_name,
            {"mapped_col": "src_id"}, {"mapped_col": {"type": "TEXT"}},
        ...
        self.assertIn({'row': '1', 'error': 'Validation error', 'data': '{"mapped_col": "mapped_value"}'}, results.errors)
