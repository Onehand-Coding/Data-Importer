...
class TestExcelImporter(unittest.TestCase):
    ...
    def _create_temp_xlsx(self, data_rows_with_headers: list[list[any]], file_name: str = "test.xlsx") -> Path:
    ...
    def test_get_headers_empty_file(self):
        ...
        with self.db_manager as db:
            importer = ExcelImporter(db)
            with self.assertRaisesRegex(ValueError, "appears to have no headers or is empty"):
    ...
    def test_get_preview_valid_excel(self):
        data = [["Name", "Email", "Value"], ["Alice", "a@ex.com", 100], ["Bob", "b@ex.com", 200.50]]
        xlsx_path = self._create_temp_xlsx(data)
        with self.db_manager as db:
            importer = ExcelImporter(db)
            preview_df = importer.get_preview(xlsx_path, num_rows=2) # Corrected num_rows to match data
    ...
    def test_process_import_valid_excel(self):
        data = [["full_name", "email", "age"], ["Alice", "alice@example.com", 30.0], ["Bob", "bob@example.com", 25.5]]
        ...
        with self.db_manager as db:
            ...
            results: ImportResult = importer.process_import(xlsx_path, table_name, mapping, schema_info_for_validation)
        ...
        with self.db_manager as db:
            cursor = db.execute(f"SELECT name, email_addr, age_val FROM {table_name} ORDER BY name")
    ...
    def test_process_import_duplicate_email_xlsx(self):
        data = [["Name", "Email"], ["Alice", "common@example.com"], ["Bob", "bob@example.com"], ["Charlie", "common@example.com"]]
        ...
        with self.db_manager as db:
            ...
            self.assertTrue(db.create_dynamic_table(table_name, schema))
            results = importer.process_import(xlsx_path, table_name, mapping, schema_info_for_validation)
