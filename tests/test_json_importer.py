...
class TestJSONImporter(unittest.TestCase):
    ...
    def _create_temp_json(self, data: Any, suffix='.json') -> Path:
        ...
        with NamedTemporaryFile(mode='w', suffix=suffix, delete=False, dir=self.temp_dir.name, encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False) # ensure_ascii=False for wider character support
            return Path(f.name)
    ...
    def test_get_headers_not_a_list(self):
        ...
        with DatabaseManager(str(self.db_path)) as db:
            importer = JSONImporter(db)
            # Use assertRaisesRegex for more specific error checking
            with self.assertRaisesRegex(ValueError, "JSON file must contain a list of objects"):
                 importer.get_headers(json_path)
    ...
    def test_get_preview(self):
        ...
        self.assertListEqual(df_preview.columns.tolist(), ['extra', 'id', 'name', 'value'])
        ...
        self.assertEqual(df_preview.iloc[0]['extra'], '') # Check filled value for missing key
    ...
    def test_full_import_process(self):
        ...
        with DatabaseManager(str(self.db_path)) as db:
            ...
            # 2. Perform import
            results: ImportResult = importer.process_import(json_path, table_name, mapping, schema_info)
            ...
            cursor = db.execute(f'SELECT name, state FROM "{table_name}" WHERE id_num = 102')
