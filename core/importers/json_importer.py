...
try:
    ...
    logger.info("core.utils.common_utils.is_valid_email not found. Using a basic placeholder for email validation.")
...
class JSONImporter(BaseImporter):
    ...
    def __init__(self, db_manager: DatabaseManager, encoding: str = 'utf-8'):
        ...
        self.file_path_loaded: Optional[Path] = None # Tracks the path of the currently loaded file data
    ...
    def _load_json_data(self, file_path: Path) -> None:
        ...
        if file_path.stat().st_size == 0:
            logger.warning(f"JSON file is empty (0 bytes): {file_path.name}. Treating as empty dataset.")
            self.data = []
            return
        ...
        try:
            ...
            err_msg = f"Could not read or parse JSON file '{file_path.name}'. Error: {str(e)}"
            logger.error(err_msg, exc_info=True) # Log full traceback for unexpected errors
            raise ValueError(err_msg) from e
        ...
        if not self.data:
            logger.info(f"JSON file {file_path.name} loaded successfully but contains an empty list (no records).")
        else:
            logger.info(f"Successfully loaded {len(self.data)} records from {file_path.name}.")
    ...
    def get_headers(self, file_path: Path) -> List[str]:
        ...
        if self.data is None: # Should ideally not be reached if _ensure_data_loaded works
             logger.error(f"Data is None after attempting to load {file_path.name}. Cannot extract headers.")
             return []

        if not self.data: # Empty list in JSON
            logger.info(f"No data items to extract headers from in {file_path.name} (JSON list is empty).")
            return []
        ...
        if not sorted_headers and self.data:
            logger.warning(f"No headers found in {file_path.name}, though data was present. "
                           "This might mean all JSON objects were empty or non-dictionary items were skipped.")
        else:
            logger.info(f"Extracted and sorted headers for {file_path.name}: {sorted_headers}")
    ...
    def get_preview(self, file_path: Path, num_rows: int = 5) -> pd.DataFrame:
        ...
        if self.data is None:
            logger.error(f"Preview failed: Data is None after attempting to load {file_path.name}.")
            return pd.DataFrame()

        headers = self.get_headers(file_path)

        if not self.data:
            logger.info(f"No data to preview from {file_path.name} (JSON list is empty).")
            return pd.DataFrame(columns=headers)

        valid_data_for_preview = [item for item in self.data if isinstance(item, dict)]

        if not valid_data_for_preview:
            logger.info(f"No dictionary items found in {file_path.name} to generate a preview.")
            return pd.DataFrame(columns=headers)

        preview_list = valid_data_for_preview[:num_rows]

        if not preview_list and num_rows > 0:
             logger.info(f"No dictionary items available in the first {num_rows} records of {file_path.name} for preview.")
             return pd.DataFrame(columns=headers)

        df = pd.DataFrame(preview_list, columns=headers)
        df = df.fillna('').astype(str)

        logger.info(f"Generated preview with {len(df)} rows and columns {df.columns.tolist()} from {file_path.name}")
        return df
    ...
    def read_data(self, file_path: Path) -> Iterator[Dict[str, Any]]:
        ...
        if self.data is None:
            logger.error(f"Data not loaded for {file_path.name} prior to reading. Cannot yield records.")
            return iter([])
        ...
        logger.info(f"Starting to read and yield records from JSON file: {file_path.name}. Total items initially: {len(self.data)}")
        num_skipped = 0
        for i, item in enumerate(self.data):
            if not isinstance(item, dict):
                logger.warning(
        ...
        if num_skipped > 0:
            logger.warning(f"Skipped a total of {num_skipped} non-dictionary items while reading data from {file_path.name}.")
        logger.info(f"Finished yielding records from {file_path.name}. Yielded {len(self.data) - num_skipped} records.")
