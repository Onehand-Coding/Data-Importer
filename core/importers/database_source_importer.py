...
class DatabaseSourceImporter:
    ...
    def __init__(self): # No target_db_manager here anymore
        ...
        self.target_db_manager: Optional[DatabaseManager] = None # Will be set by process_import_to_target
    ...
    def _get_quoted_identifier(self, identifier: str) -> str:
        # ... (no changes from before) ...
        if not self.source_engine: return f'"{identifier}"'
        dialect_name = self.source_engine.name
        if dialect_name == 'mysql' or dialect_name == 'mariadb': return f"`{identifier}`"
        elif dialect_name == 'postgresql' or dialect_name == 'sqlite': return f'"{identifier}"'
        else: logger.warning(f"Unknown dialect '{dialect_name}' for quoting, defaulting to double quotes."); return f'"{identifier}"'
                ...
                if is_query:
                    ...
                    if "limit" not in limited_query_str.lower():
                        limited_query_str = f"SELECT * FROM ({limited_query_str}) AS __subquery_for_headers LIMIT 1"
                         ...
                         limited_query_str = text(f"SELECT * FROM ({limited_query_str}) AS __subquery_for_headers LIMIT 1") # Ensure text for safety
    ...
    def _map_row_to_snippet_str(self, data_row: Optional[Dict[str, Any]], max_len: int = 100) -> str:
        ...
        try: snippet = json.dumps({k: str(v) for k, v in data_row.items()})
    ...
    def _validate_target_row(self, mapped_target_row: Dict[str, Any], row_number: Optional[int], schema_info_for_validation: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        for req_field in required_fields:
            ...
            for target_key_in_map, source_val_in_map in self.column_mapping.items():
                if target_key_in_map == req_field: original_source_header = source_val_in_map; break
        ...
        for target_db_field_name, value in mapped_target_row.items():
            ...
            if isinstance(column_specific_target_schema, dict): is_email_field_in_target = column_specific_target_schema.get('is_email', False)
            if not is_email_field_in_target and "email" in target_db_field_name.lower(): is_email_field_in_target = True
    ...
    def _insert_data_to_target(self, sanitized_target_table_name: str, data_to_insert: Dict[str, Any], result: ImportResult, row_number: Optional[int]) -> bool:
        ...
        if not data_to_insert: result.add_error(row_number, "No data to insert (empty mapped row).", None); return False
        ...
        try:
            ...
            if cursor: result.rows_inserted += 1; return True
            result.add_error(row_number, "Target DB insert failed.", self._map_row_to_snippet_str(data_to_insert)); return False
        except sqlite3.IntegrityError as ie:
            logger.warning(f"Target DB Integrity error for {sanitized_target_table_name} row {row_number}: {ie}")
            error_msg = f"Target DB Integrity Error: {ie}" # Simplified for now
            result.add_error(row_number, error_msg, self._map_row_to_snippet_str(data_to_insert)); return False
        except Exception as e:
            logger.exception(f"Unexpected target DB insert error for {sanitized_target_table_name} row {row_number}: {e}")
            result.add_error(row_number, f"Unexpected Target DB Insert Error: {str(e)}", self._map_row_to_snippet_str(data_to_insert)); return False
    ...
    def process_import_to_target(
        ...
        if target_db_manager is None:
            logger.error("Target DatabaseManager instance is required for import process.")
            ...
            raise ValueError("Target DatabaseManager instance cannot be None for import process.")
        self.target_db_manager = target_db_manager # Set the target manager for this operation
        ...
        if not self.target_db_manager or not self.target_db_manager.connection:
            results.add_error(0, "Target DatabaseManager is not connected or not provided.")
            return results
