...
class ExcelImporter(BaseImporter):
    ...
    def get_headers(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            if sheet.max_row > 0:
                try: # Try to get the first row cells
                    ...
                    if first_row_cells: # Check if the row object is not None
                        ...
                        headers = [h for h in headers if h] # Remove empty strings if any
                except IndexError:
                    logger.warning(f"openpyxl could not access row 1 directly for {file_path.name} (sheet might be empty or have no cells in row 1). Treating as no headers by openpyxl.")
                    headers = [] # Ensure headers is empty to trigger pandas or final error

            if not headers:
                logger.info(f"No headers found via openpyxl direct row access for {file_path.name}. Trying pandas fallback.")
                try:
                    df_header = pd.read_excel(file_path, sheet_name=0, nrows=0, engine='openpyxl')
                    headers = [str(h).strip() for h in df_header.columns if str(h).strip()]
                except pd.errors.EmptyDataError:
                     logger.warning(f"Pandas read_excel found EmptyDataError for {file_path.name} (file is likely empty).")
                     headers = []
                except Exception as pd_err:
                    logger.warning(f"Pandas fallback for headers failed for {file_path.name}: {pd_err}")
                    headers = []

            if not headers:
                raise ValueError(f"Excel file '{file_path.name}' appears to have no headers or is empty.")
            ...
            logger.exception(f"General failure to read headers from Excel file {file_path}: {e}")
            # Raise a consistent error message if an unexpected exception occurs
            raise ValueError(f"Could not read headers from Excel file {file_path.name} due to an unexpected error: {e}") from e
    ...
    def get_preview(self, file_path: Path, num_rows: int = 5) -> pd.DataFrame:
        ...
        try:
            ...
            return df.astype(str) # Convert all to string for consistent preview display
        except Exception as e:
            logger.exception(f"Failed to generate preview for Excel file {file_path}: {e}")
    ...
    def read_data(self, file_path: Path) -> Generator[Dict[str, Any], None, None]:
        ...
        try:
            workbook = openpyxl.load_workbook(file_path, read_only=True, data_only=True) # data_only=True to get values, not formulas
            sheet = workbook.active

            iterator = sheet.rows

            try:
                header_row_values = [str(cell.value).strip() if cell.value is not None else "" for cell in next(iterator)]
            except StopIteration:
                logger.warning(f"Excel file {file_path.name} is empty or has no header row.")
                ...
                for header, value in zip(header_row_values, values):
                    if header: # Only include columns with actual headers
                        if isinstance(value, str):
                            ...
                            row_data[header] = None # Or "" if you prefer empty string for None
                        else:
                            row_data[header] = value

                if not any(row_data.values()): # Skip entirely empty rows
                    logger.debug(f"Skipping empty row {row_count + 1} in {file_path.name}")
                    continue

                yield row_data

        except Exception as e:
            logger.exception(f"Failed during data reading from Excel file {file_path}: {e}")
            raise ValueError(f"Error reading data from Excel file {file_path.name}: {e}") from e
